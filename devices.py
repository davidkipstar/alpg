...
class DeviceFan(Device):
    def simulate(self, timeintervals, day, occupancy, occupancyPerson, person):
        #                                                             , persons)
        #for p in persons:
        #   p.Workdays ...
        #
        ...
        for m in range(0, timeintervals):
            at_home = False 
            for i in range(len(occupancyPerson)):
                #print(occupancyPerson[i])
                if occupancyPerson[i][m]:
                    at_home = True  #i th person and m moment
            
            if dayOfWeek in person.Workdays:
                if person.WorkdayWakeUp_Avg > m and m < person.WorkdayLeave_Avg:
                    FanProfile[m] = FanProfile[m] + self.Consumption
                elif m > person.WorkdayArrival_Avg and person.WorkdayBedTime_Avg:
            ...
            if(hasInductionCooking):
                ...
                for m in range(startCooking, startCooking+cookingDuration):
                    if m < 6+startCooking:
                        ...
                        CookingProfile[m] += round(
                            config.ConsumptionInductionStove*(inductionRatio/10))
